...
{
    if (_m == nil) {
        _m = [self.device.model stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    }
    return _m;
}
...
{
    if (_o == nil) {
        _o = [self.device.systemName stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    }
    return _o;
}
 - (NSString *)v
...
{
    if (_cv == nil) {
        _cv = [[[NSBundle mainBundle] infoDictionary] objectForKey:@"CFBundleVersion"];
    }
    return _cv;
}
...
{
    if (_macid == nil) {
        _macid = [self.device WK_macaddressMD5];
    }
    return _macid;
}
...
{
    if (_ostype2 == nil) {
        _ostype2 = self.device.WK_ostype;
    }
    return _ostype2;
}
...
- (NSString *)cid
{
    return [[WKLocationManager sharedInstance] currentCityId];
}
...
- (NSString *)geo
{
    CLLocationCoordinate2D coordinate = [[WKLocationManager sharedInstance].locatedCityLocation coordinate];
    return [NSString stringWithFormat:@"%f, %f", coordinate.latitude, coordinate.longitude];
}

- (NSString *)gcid
{
    return [WKLocationManager sharedInstance].locatedCityId;
}
...
{
    if (_pmodel == nil) {
        _pmodel = [[UIDevice currentDevice] WK_machineType];
    }
    return _pmodel;
}
...
{
    static WKAppContext *sharedInstance;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedInstance = [[WKAppContext alloc] init];
        [[AFNetworkReachabilityManager sharedManager] startMonitoring];
        [WKLocationManager sharedInstance];
    });
    return sharedInstance;
}

- (void)configWithChannelID:(NSString *)channelID appName:(NSString *)appName appType:(WKAppType)appType ccid:(NSString *)ccid
{
