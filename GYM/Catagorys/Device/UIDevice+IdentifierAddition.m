...
- (NSString *)WK_uuid
{
    ...
    if (uuid.length == 0) {
        [[NSUserDefaults standardUserDefaults] setObject:[self WK_createUUID] forKey:key];
...
- (NSString *) WK_udid
{
    NSString *udid = [[WKUDIDGenerator sharedInstance] UDID];
    if (udid.length==0) {
        udid = [self WK_uuid];
        [[WKUDIDGenerator sharedInstance] saveUDID:udid];
    }
    return udid;
}
...
- (NSString *) WK_macaddressMD5{
    ...
    if (macid.length == 0) {
        NSString *macaddress = [[UIDevice currentDevice] WK_macaddress];
        macid = [macaddress WK_md5];
        if (!macid){
...
- (NSString *) macaddress
{
    return self.WK_macaddress;
}
- (NSString *) macaddressMD5
{
    return self.WK_macaddressMD5;
}
- (NSString *) machineType
{
    return self.WK_machineType;
}
- (NSString *) ostype//显示“ios6，ios5”，只显示大版本号
{
    return self.WK_ostype;
}
- (NSString *) createUUID
{
    return self.WK_createUUID;
}
